...
const Cart = () => {
    ...
    useEffect( () => {
        ...
        for(let i = 0; i<cartItems.length ; i++){
            tot += cartItems[i].card.info.defaultPrice != undefined ? (cartItems[i].card.info.defaultPrice * cartItems[i].itemQuantity) : (cartItems[i].card.info.price * cartItems[i].itemQuantity)  ;
        }
        setItemTotal(tot);
    },[cartItems])
